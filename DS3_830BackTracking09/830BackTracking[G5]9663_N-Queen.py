...
cnt=0 # if function reaches to last row(x=N), that's the case where the Quenes all spotted 
#  x=N<-- out of the range, but thats the condition to escape recursion,here 
def DFS(x):
    global cnt 
    if x==N:
        cnt+=1
    ...
    #    just to check there is anywhere to put the Quene traversing culumn(y) on that row(x) 
        ...
        # it recursively gets back to here if it meets all condition, 
