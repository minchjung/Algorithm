...
const gossipProtocol2 = function (village, num) {
  ...
  // Get the candidated one that has '2'
  for(let i = 0 ; i < R; i++){
    for(let j = 0 ; j < C; j++){
  ...
  // Search shortest time-cost to spread them all out
  for(let candidate of comb){ 
    ...
    let Q =[]
    // push the picked all into Q first
    ...
    while(Q.length > 0){
      ...
      for(let d = 0 ; d < 4; d++){
        ...
        if( vis[nxtR][nxtC] || village[nxtR][nxtC] === '0') continue; // visit both '1' and '2' 
        vis[nxtR][nxtC] = true; 
