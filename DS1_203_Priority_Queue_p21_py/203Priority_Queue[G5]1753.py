...
while hq: 
    ...
    for nodePair in board[now[1]]: # now 노드에 연결된 [가중치,다른노드] 정보를 탐색
        ...
        heapq.heappush(hq, [ isUsed[nodePair[1]], nodePair[1]]) #<-- 위처럼 탐색 노드 정보를 그대로 넣어주지 않고, 바뀌면 바뀐 거리정보로 
        # 해당 노드까지의 최단거리 가중치 거리를 갱신해서 힙자료에 넣어주면 조금더 입구컷이 빨라진다  
        # 원래 코드 : heapq.heappush(hq, nodePair)
